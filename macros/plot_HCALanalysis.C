...
void plot_HCALanalysis(TString sample="Singlemu", TString mode="rate", Bool_t debug=false) {

  TString ifile, data;

  if (sample=="Minbias") {
    ifile="l1Ntuple_ZeroBias_RunD_withEcalTPs.root";
    //    ifile="l1Ntuple_ZeroBias_RunD_1210.root";
    data="MinBias (run 2012D)";
  } else if (sample=="Singlemu") {
    ifile="l1Ntuple_SingleMu_RunD_oldPMTs.root";
 ...
 //  // Central + Tau jets
  ...
  if (debug) {
    ...
    TH2D *ietaJet_iphiJet_central=readHist2D(dirname+"ietaJet_iphiJet_central",ifile,0);
    TH2D *ietaJet_iphiJet_hf_central=readHist2D(dirname+"ietaJet_iphiJet_hf_central",ifile,0);
    TH2D *ietaJet_iphiJet=readHist2D(dirname+"ietaJet_iphiJet",ifile,0);
    TH2D *ietaJet_iphiJet_hf=readHist2D(dirname+"ietaJet_iphiJet_hf",ifile,0);
  ...
  // For L1Jet ET>240 (BX=0)
    ...
    leg->AddEntry(iphiTP_H,"Events with BX=0 L1jet of saturated E_{T}","LF");
    ...
    tp3->cd(1); 
    TLegend *leg = legend();
    leg->SetHeader("hcalTPs for all events");
 ...
 // ----------> Central triggers
  ...
  if (mode=="rate"){
    c1=getaCanvas("c1"); c1->Divide(1,2); 
    pad1 = new TPad("pad1", "The pad with the function",0.05,0.4,0.95,0.95);
  } else {
    c1=getanotherCanvas("c1");
    pad1 = new TPad("pad1", "The pad with the function",0.05,0.05,0.95,0.95);
  }
...
//   etaJet_l->SetLineColor(1); etaJet_l->SetLineColor(4);   etaJet_l->SetLineStyle(3);
  ...
  if (mode=="rate"){
    c3=getaCanvas("c3"); c3->Divide(1,2); 
    pad5 = new TPad("pad5", "The pad with the function",0.05,0.4,0.95,0.95);
  } else {
    c3=getanotherCanvas("c3");
    pad5 = new TPad("pad5", "The pad with the function",0.05,0.05,0.95,0.95);
  }

 // TCanvas *c3=getaCanvas("c3");
 // if (mode=="rate") { c3->Divide(1,2); }
